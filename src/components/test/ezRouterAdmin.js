...
import usersCreate from "../admin/adminCreateUser"
...
class ezRouter extends Component {
    ...
    onClick(e){
        if(e.target.name === "Main"){
            ...
            this.props.history.push({
                pathname: "/" + localStorage.getItem("role") + "/users",
                // state: { detail: username, name:result.data.name, id:result.data.id, role:result.data.role }
            })
            window.location.reload();

        }else if(e.target.name === "Create Users"){
            this.props.history.push({
                pathname: "/" + localStorage.getItem("role") + "/createUsers",
                // state: { detail: username, name:result.data.name, id:result.data.id, role:result.data.role }
            })
            window.location.reload();
        ...
        return(
            <div style={{paddingLeft:10}}>
                <Nav>
                    ...
                    <NavItem>
                        <NavLink><a name={"Users"} onClick={this.onClick}>Users</a></NavLink>
                    </NavItem>
                    <NavItem>
                        <NavLink><a name={"Create Users"} onClick={this.onClick}>Create Users</a></NavLink>
                    </NavItem>
                    <NavItem>
                ...
                <div>
                    ...
                    <Route path={this.props.match.url + '/createUsers'} component={usersCreate}/>
